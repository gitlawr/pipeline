...
func (s *Server) CreatePipeline(rw http.ResponseWriter, req *http.Request) error {
	...
	if ppl.Templates != nil {
		templateContent := ppl.Templates["pipeline.yaml"]
		if templateContent == "" {
			return fmt.Errorf("expected import template with 'pipeline.yaml' key")
		}
		if err := yaml.Unmarshal([]byte(templateContent), ppl); err != nil {
			return err
		}
	}
