...
func NewRouter(s *Server) *mux.Router {
	...
	//scm accounts
	router.Methods(http.MethodGet).Path("/v1/accounts").Handler(f(schemas, s.ListAccounts))
	router.Methods(http.MethodGet).Path("/v1/accounts/{id}").Handler(f(schemas, s.GetAccount))
	router.Methods(http.MethodPost).Path("/v1/accountsdebug/{id}").Handler(f(schemas, s.DebugCreate))
	router.Methods(http.MethodGet).Path("/v1/accounts/{id}/repos").Handler(f(schemas, s.GetCacheRepos))
	//settings
	...
	for name, actions := range accountActions {
		router.Methods(http.MethodPost).Path("/v1/accounts/{id}").Queries("action", name).Handler(actions)
	}
	return router
}
